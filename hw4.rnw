...
The line is the original time series plot, and the fitted values are the points. The model almost perfectly describes the obseved CO2 concentrations because all the points lie almost exactly on the time series line.

<<prob5, fig.height = 4, fig.width = 6, fig.align = 'center'>>=
par(mfrow = c(1,1))
plot.ts(ts_x[,3], xlab = "Time Series Index", ylab = "[CO2] Value")
points(lm_x$fitted.values)
@
